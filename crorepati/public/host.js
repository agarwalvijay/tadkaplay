...
function renderScoreboard(el, players, opts = {}) {
  ...
  players.forEach((p, i) => {
    ...
    const row = document.createElement('div'); row.className = 'sb-row' + (opts.winner && i === 0 ? ' winner' : '') + (off ? ' offline' : ''); row.style.animationDelay = `${i * 0.06}s`;
    ...
    row.append(rank, avatarEl(p.avatar, off ? 'offline' : ''), name, score); el.appendChild(row);
  });
}
